...
public class FacturaController {
    ...
    final String PEDIDOS_ANULADOS = VIEW_PATH + "/pedidoNull";
    final String PEDIDOS_RECEPCIONADOS = VIEW_PATH + "/pedidoFinal";
    ...
    */
    ...
    public String mostrarGrilla(Model model, RedirectAttributes attributes) {
        ...
        if(consultar) {
            List<Compra> compraPend = compraRepository.findByEstado("PENDIENTE");
            List<Compra> compraParcial = compraRepository.findByEstado("PARCIALMENTE RECEPCIONADO");
            List<Compra> compraRecep = compraRepository.findByEstado("RECEPCIONADO");
            List<Compra> compraAnul = compraRepository.findByEstado("ANULADO");
            List<Compra> compra = compraRepository.findByEstado("PENDIENTE");
            List<Compra> compraPar = compraRepository.findByEstado("PARCIALMENTE RECEPCIONADO");
            List<Compra> comprasCombinadas = new ArrayList<>();
            comprasCombinadas.addAll(compra);
            comprasCombinadas.addAll(compraPar);
            model.addAttribute("listPend", compraPend);//lista los productos
            model.addAttribute("listParc", compraParcial);//LISTA PEDIDOS PENDIENTES
            model.addAttribute("listRecep", compraRecep);//LISTA PEDIDOS PARCIALMENTE ENTREGADOS
            model.addAttribute("listAnul", compraAnul);//LISTA PEDIDOS ENTREGADOS
            model.addAttribute("listCr", comprasCombinadas);//LISTA PEDIDOS ANULADOS
    ...
    @GetMapping("/pedidoNull/{id}")
    public String formNull(Model model,@PathVariable Long id) {
        ...
        if(eliminar) {
            return PEDIDOS_ANULADOS;
        } else {
            return FALTA_PERMISO_VIEW;
        }
    }

     @GetMapping("/pedidoFinal/{id}")
    public String formFinal(Model model,@PathVariable Long id) {
        boolean eliminar = true;
        boolean asignarRol = usuarioService.tienePermiso("asignar-rol-" + VIEW);
        //Venta venta;
        Compra compra;
    ...
    public String formEditar(@PathVariable Long id, Model model) {
        ...
        try {
            ...
            // compraService.getDTO(compra, compraDTO);
            model.addAttribute("compra", compra);
            model.addAttribute("id_compra", id);
            //solo tenia esta linea de abajo antes
            //compra = compraService.existeCompra(idCompra);
        } catch(Exception e) {
            return RD_FORM_VIEW;
    ...
    public String actualizarObjeto(@PathVariable Long id, @RequestParam(value="estado") String estado,
                    @RequestParam(value="motivo") String motivo ,@RequestParam(value="documentos") String documentos,
                    RedirectAttributes attributes) {
        this.operacion = "actualizar-";
        //Compra venta;
        Compra compra;
        String[] arrDocumento = documentos.split("\\|");
        ...
        if(compra != null) {
            ...
            if(estado.equals("ANULADO")){
                ...
                compra.setUsuarioIdAnulacion(usuario.getIdUsuario().toString());
            }
            if(estado.equals("RECEPCIONADO")){
                compra.setUsuarioIdRecepcion(usuario.getIdUsuario().toString());
            ...
            for (String docCrudo : arrDocumento) {
                ...
                if(!elementos[0].equals("0")){
                    docDet.setIdFactura(new Long(elementos[0]));
                }
                docDet.setNroDocumento(elementos[1]);
                docDet.setTipo(new String(elementos[2]));
